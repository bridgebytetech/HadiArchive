...
export default function AdminWritingsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["adminWritings", page],
    queryFn: () => writingService.adminGetAll(page, 20),
  });

  const togglePublish = useMutation({
    mutationFn: (id: string) => writingService.togglePublish(id),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => writingService.delete(id),
    onSuccess: () => {
  ...
  const columns: Column<Writing>[] = useMemo(
    () => [
      {
        key: "title",
        header: "Writing",
        cell: (item) => (
          <div className="min-w-0">
            ...
            <div className="text-xs text-muted-foreground">
              {item.writingType || "—"} • {item.date ? formatDateEn(item.date) : "—"}{" "}
              {item.publishedIn ? `• ${item.publishedIn}` : ""}
            </div>
          </div>
        ),
      },
        ...
        cell: (item) => (
          <div className="flex justify-end gap-2">
            <Button asChild variant="outline" size="sm">
              <Link href={`/admin/writings/${item.id}/edit`}>Edit</Link>
            </Button>
  ...
  return (
    <AdminLayout>
      <AdminPageHeader
        title="Writings"
        description="Manage writings/articles"
        actionLabel="Add Writing"
      ...
      <DataTable
        ...
        data={data?.content || []}
        isLoading={isLoading}
        emptyMessage="No writings found"
      />
