...
export default function AdminPoemsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["adminPoems", page],
    queryFn: () => poemService.adminGetAll(page, 20),
  });

  const togglePublish = useMutation({
    mutationFn: (id: string) => poemService.togglePublish(id),
    onSuccess: () => {
  ...
  const toggleFeatured = useMutation({
    mutationFn: (id: string) => poemService.toggleFeatured(id),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => poemService.delete(id),
    onSuccess: () => {
  ...
  const columns: Column<Poem>[] = useMemo(
    () => [
      {
        key: "title",
        header: "Poem",
        cell: (item) => (
          <div className="min-w-0">
            ...
            <div className="text-xs text-muted-foreground">
              {item.date ? formatDateEn(item.date) : "—"} {item.dedicatedTo ? `• Dedicated: ${item.dedicatedTo}` : ""}
            </div>
          </div>
        ),
      },
      {
        ...
        header: "Flags",
        cell: (item) => (
          <div className="text-xs text-muted-foreground">{item.featured ? "Featured" : "—"}</div>
        ),
      },
      {
        key: "status",
        ...
        cell: (item) => (
          <div className="flex justify-end gap-2">
            <Button asChild variant="outline" size="sm">
              <Link href={`/admin/poems/${item.id}/edit`}>Edit</Link>
            </Button>
  ...
  return (
    <AdminLayout>
      <AdminPageHeader
        title="Poems"
        description="Manage poems (publish/feature)"
        actionLabel="Add Poem"
        actionHref="/admin/poems/new"
        actionIcon={<Plus className="h-4 w-4" />}
      />

      <DataTable columns={columns} data={data?.content || []} isLoading={isLoading} emptyMessage="No poems found" />
