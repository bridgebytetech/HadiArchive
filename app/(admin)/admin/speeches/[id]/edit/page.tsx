...
export default function EditSpeechPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["adminSpeech", id],
    queryFn: () => speechService.adminGetById(id),
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: any) => speechService.update(id, payload),
    onSuccess: () => {
      toast.success("Updated");
      qc.invalidateQueries({ queryKey: ["adminSpeeches"] });
      qc.invalidateQueries({ queryKey: ["adminSpeech", id] });
    },
  ...
  if (isLoading || !data) {
    return (
      <AdminLayout>
        <AdminPageHeader title="Edit Speech" backHref="/admin/speeches" />
