...
import { docService } from "@/services/documentService"; // NOTE: নিচে services ফিক্স দিলাম
...
export default function AdminDocumentsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["adminDocuments", page],
    queryFn: () => docService.adminGetAll(page, 20),
  });

  const togglePublish = useMutation({
    mutationFn: (id: string) => docService.togglePublish(id),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => docService.delete(id),
    onSuccess: () => {
  ...
  const columns: Column<DocDocument>[] = useMemo(
    () => [
      {
        key: "title",
        header: "Document",
        cell: (item) => (
          <div className="min-w-0">
            ...
            <div className="text-xs text-muted-foreground">
              {item.documentType || "—"} • {item.date ? formatDateEn(item.date) : "—"}{" "}
        ...
        cell: (item) => (
          <div className="flex justify-end gap-2">
            <Button asChild variant="outline" size="sm">
              <Link href={`/admin/documents/${item.id}/edit`}>Edit</Link>
            </Button>
  ...
  return (
    <AdminLayout>
      ...
      <DataTable columns={columns} data={data?.content || []} isLoading={isLoading} emptyMessage="No documents found" />
